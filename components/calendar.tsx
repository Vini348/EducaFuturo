...
export const CalendarBody = ({ features, children, onSelectDate }: CalendarBodyProps) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        key={day}
        className="relative flex h-full w-full flex-col gap-1 p-1 text-muted-foreground text-xs cursor-pointer hover:bg-gray-100"
        onClick={handleClick}
      >
        {day}
        <div>{featuresForDay.slice(0, 3).map((feature) => children({ feature }))}</div>
        {featuresForDay.length > 3 && (
          <span className="block text-muted-foreground text-xs">+{featuresForDay.length - 3} more</span>
        )}
      </div>,
    )
  }
